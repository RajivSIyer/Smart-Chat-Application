...
class response(object): #server to client
    ...
    def encode_Json(self, resp):
        ...
        if self.payload != '' and self.payload != 'None':
            return {"cmd":self.cmd, "resp_code":self.resp_code, "req_id":self.req_id, "msg":json.dumps(self.msg), "payload":self.payload.encode_Json()}
        else:
            return {"cmd":self.cmd, "resp_code":self.resp_code, "req_id":self.req_id, "msg":json.dumps(self.msg), "payload":''}
